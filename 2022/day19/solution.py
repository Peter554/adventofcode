...
def should_build_robot(
    robot: Mineral,
    ms: MiningState,
    max_robots_needed: dict[Mineral, int],
    max_timestep: int,
) -> bool:
    if robot not in max_robots_needed:
        return True
    if ms.robots[robot] >= max_robots_needed[robot]:
        return False
    if (
        ms.minerals[robot] + (max_timestep - ms.timestep) * ms.robots[robot]
        >= (max_timestep - ms.timestep) * max_robots_needed[robot]
    ):
        return False
    return True
...
def max_blueprint_geodes(bp: Blueprint, max_timestep: int) -> int:
    ...
    # since we can only build one robot per timestep, there is no need to build more than this!
    max_robots_needed: dict[Mineral, int] = {}
    max_robots_needed[Mineral.ORE] = max(
        robot_cost.get(Mineral.ORE, 0) for robot_cost in bp.robot_costs.values()
    )
    max_robots_needed[Mineral.CLAY] = max(
        robot_cost.get(Mineral.CLAY, 0) for robot_cost in bp.robot_costs.values()
    )
    max_robots_needed[Mineral.OBSIDIAN] = max(
    ...
    while q:
        ...
        for next_robot_to_build in Mineral:
            if not should_build_robot(
                next_robot_to_build, ms, max_robots_needed, max_timestep
            ):
                continue
