...
ValveTour = tuple[tuple[str, ...], tuple[str, ...]]
...
def tweak_valve_tour(vt: ValveTour) -> ValveTour:
    t = (*vt[0], *vt[1])
    start_idx = random.randint(0, len(t) - 2)
    end_idx = random.randint(start_idx + 1, len(t) - 1)
    t = (*t[:start_idx], *reversed(t[start_idx : end_idx + 1]), *t[end_idx + 1 :])
    split_idx = random.randint(0, len(t) - 1)
...
def solve(file_path: str) -> int:
    ...
    l = len(valves) // 2
    initial_valve_tour = (tuple(valves)[:l], tuple(valves)[l:])
    final_valve_tour = anneal(
        initial_state=initial_valve_tour,
        state_cost=lambda vt: -1 * get_valve_tour_flow(valves, tunnel_paths, 26)(vt),
        tweak_state=tweak_valve_tour,
        initial_temperature=1000,
        iterations_per_temperature=1000,  # 5000
        temperature_decay_factor=0.95,  # 0.99
    )
    return get_valve_tour_flow(valves, tunnel_paths, 26)(final_valve_tour)
